        ...
        Dir.chdir(module_dir) do
          ...
          if modified_gemfile == buffer.source
            File.open('Gemfile', 'a') do |f|
              f.puts "gem puppetlabs-'rspec-puppet', :path => '#{gem_root}'"
            end
          else
...
class GemfileRewrite < Parser::TreeRewriter
  def on_send(node)
    ...
    if method_name == :gem
      gem_name = args.first
      if gem_name.type == :str && gem_name.children.first == 'puppetlabs-rspec-puppet'
        gem_root = File.expand_path(File.join(__FILE__, '..', '..', '..', '..'))
        replace(node.location.expression, "gem 'puppetlabs-rspec-puppet', :path => '#{gem_root}'")
      end
    end
